...
{
    sealed class ActionProfileWarheadDetonate : ActionProfile<IMyWarhead>
    {

        public override IEnumerable<string> ActionNames => new[] { "Detonate" };

        public override Action<IMyWarhead, IDictionary<string, string>> OnActionCallback =>
            (block, args) => block.Detonate();
